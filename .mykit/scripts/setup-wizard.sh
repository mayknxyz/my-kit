...
#   validation settings, and release settings. Writes configuration to
#   .mykit/config.json using atomic file operations.
#
# USAGE:
...
read_existing_config() {
  ...
  CURRENT_BRANCH=$(grep -o '"branch": *"[^"]*"' "$CONFIG_FILE" 2>/dev/null | head -1 | cut -d'"' -f4) || true
  CURRENT_AUTO_ASSIGN=$(grep -o '"autoAssign": *[^,}]*' "$CONFIG_FILE" 2>/dev/null | head -1 | sed 's/.*: *//' | tr -d ' ') || true
  CURRENT_DRAFT_MODE=$(grep -o '"draftMode": *[^,}]*' "$CONFIG_FILE" 2>/dev/null | head -1 | sed 's/.*: *//' | tr -d ' ') || true
  CURRENT_AUTO_FIX=$(grep -o '"autoFix": *[^,}]*' "$CONFIG_FILE" 2>/dev/null | head -1 | sed 's/.*: *//' | tr -d ' ') || true
  CURRENT_VERSION_PREFIX=$(grep -o '"versionPrefix": *"[^"]*"' "$CONFIG_FILE" 2>/dev/null | head -1 | cut -d'"' -f4) || true
  CURRENT_GH_AUTH=$(grep -o '"authenticated": *[^,}]*' "$CONFIG_FILE" 2>/dev/null | head -1 | sed 's/.*: *//' | tr -d ' ') || true
...
run_wizard() {
  ...
  echo "─── Step 1/5: GitHub Authentication ───"
  ...
  echo "─── Step 4/5: Validation Settings ───"
  ...
  echo "─── Step 5/5: Release Settings ───"
  ...
  if write_config "$gh_authenticated" "$default_branch" "$auto_assign" "$draft_mode" "$auto_fix" "$version_prefix"; then
    echo ""
...
show_preview() {
  ...
  echo "  4. Configure validation settings (auto-fix)"
  echo "  5. Configure release settings (version prefix)"
  echo ""
