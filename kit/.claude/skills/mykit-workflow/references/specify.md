...
1. Fetch the issue title for branch naming:

   ```bash
   ISSUE_TITLE=$(gh issue view $issueNumber --json title --jq '.title' 2>/dev/null || echo "")
   ```

2. Generate a branch slug from the issue title:
...
3. Create the feature branch and spec directory:
...
4. The script switches to the new branch automatically. Update variables:
