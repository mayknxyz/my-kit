...
# Template file
TEMPLATE_FILE="$HOME/.claude/skills/mykit/references/templates/agent-file-template.md"
...
validate_environment() {
    ...
    if [[ -z "$CURRENT_BRANCH" ]]; then
        ...
        if [[ "$HAS_GIT" == "true" ]]; then
            ...
            log_info "Set MYKIT_FEATURE environment variable or create a feature first"
        fi
        exit 1
    fi
    ...
    if [[ ! -f "$NEW_PLAN" ]]; then
        ...
        if [[ "$HAS_GIT" != "true" ]]; then
            log_info "Use: export MYKIT_FEATURE=your-feature-name or create a new feature first"
        fi
        exit 1
    fi
...
create_new_agent_file() {
    ...
    for substitution in "${substitutions[@]}"; do
        sed_args+=(-e "$substitution")
    done
    if ! sed -i.bak "${sed_args[@]}" "$temp_file"; then
