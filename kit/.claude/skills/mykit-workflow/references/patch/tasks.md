<!-- Patch mode: optional lightweight task generation workflow -->

## Patch Mode Tasks

Generate an optional lightweight task breakdown from feature specification and/or implementation plan, or via guided conversation.

In Patch mode, creating a task breakdown is optional. For simple fixes, skip directly to `/mykit.implement`. For complex patches with multiple steps, use this to organize your work.
...
These artifacts won't be leveraged in patch mode. Proceeding with patch task generation.
```
...
- If neither exists or both are too short: `contentSource = "guided"` (trigger Step 10)
