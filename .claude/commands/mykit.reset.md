...
This command clears My Kit workflow state, allowing you to start a new workflow session without interference from previous sessions. By default, it shows a preview of what would be cleared. Use the `run` action to execute the reset.
...
|----------|----------|-------------|
| `run` | No | Execute the reset (without it, shows preview only) |
...
| `--force` | Skip preview and execute immediately (requires `run` action) |
...
- `sessionType`: Type of workflow (full, lite, quickfix)
