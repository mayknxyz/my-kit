...
#######################################
validate_shell_scripts() {
  local scripts_dir="$REPO_ROOT/.mykit/scripts"
  local exit_code=0
  ...
  for script in "${script_files[@]}"; do
    ...
    if shellcheck --severity=warning "$script" 2>&1 | grep -q "^In"; then
      # shellcheck found issues
      local issues
      issues=$(shellcheck --severity=warning "$script" 2>&1)
...
run_all_validations() {
  ...
  local shell_exit=0
  local markdown_exit=0

  # Run validations
  validate_shell_scripts || shell_exit=$?
  echo ""
  validate_markdown || markdown_exit=$?
