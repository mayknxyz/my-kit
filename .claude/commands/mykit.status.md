...
|-------|------------------------|-------------------|--------|
| Not started | Any | `/mykit.backlog select` | Select an issue to work on |
| Specification | No | `/speckit.clarify` or `/speckit.plan` | Continue specification work |
...
- If on main branch (not feature branch): Suggest `/mykit.backlog select`
...
Not on a feature branch. Use `/mykit.backlog select` to start working on an issue.
```
...
| `/mykit.backlog` | Suggested when not on a feature branch |
