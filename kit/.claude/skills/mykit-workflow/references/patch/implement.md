<!-- Patch mode: task execution with progress tracking and direct-fix fallback -->

## Patch Mode Implementation

Execute implementation tasks from tasks.md one by one with autonomous execution, progress tracking, and workflow guidance. If no tasks.md exists, fall back to direct implementation.
...
No formal task list. Implementation is done directly.

**Commands**:
- `/mykit.implement` - Describe and implement a fix
- `/mykit.tasks -c` - Create a task list first (optional)
- `/mykit.audit` - Run quality checks
- `/mykit.commit` - Create a commit
```
...
Prompt the user for what needs to be done:

Use `AskUserQuestion` tool:
- header: "Patch Description"
- question: "Describe the fix or change you want to implement."
- options:
  1. label: "Describe", description: "Provide a description of the fix or change"

Based on the user's description, autonomously implement the change:
...
- `/mykit.audit` - Run quality checks
- `/mykit.commit` - Create a commit
- `/mykit.pr -c` - Create a pull request
```
...
Display:
```
**Error**: No tasks file found. Use `/mykit.implement` to implement changes directly, or `/mykit.tasks -c` to create a task list.
```
...
**If tasks.md exists**, continue to Step 7.
