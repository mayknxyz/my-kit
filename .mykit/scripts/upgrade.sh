...
create_backup() {
    ...
    # Backup scripts
    if [[ -d "$install_dir/.mykit/scripts" ]]; then
        cp -r "$install_dir/.mykit/scripts" "$backup_path/scripts/"
    fi

    # Backup templates
    if [[ -d "$install_dir/.mykit/templates" ]]; then
        cp -r "$install_dir/.mykit/templates" "$backup_path/templates/"
    fi
...
restore_backup() {
    ...
    if [[ -d "$backup_path/scripts" ]]; then
        rm -rf "$install_dir/.mykit/scripts"
        cp -r "$backup_path/scripts" "$install_dir/.mykit/scripts"
    fi

    # Restore templates
    if [[ -d "$backup_path/templates" ]]; then
        rm -rf "$install_dir/.mykit/templates"
        cp -r "$backup_path/templates" "$install_dir/.mykit/templates"
    fi
...
download_files() {
    ...
    local tarball_url
    tarball_url="https://github.com/$MYKIT_REPO/archive/refs/tags/$version.tar.gz"

    if ! curl -sSL "$tarball_url" -o "$temp_dir/release.tar.gz" 2>/dev/null; then
...
install_files() {
    ...
    if [[ -d "$source_dir/.claude/commands" ]]; then
        # Remove old commands except speckit (if present)
    ...
    # Install scripts
    if [[ -d "$source_dir/.mykit/scripts" ]]; then
        cp -r "$source_dir/.mykit/scripts"/* "$install_dir/.mykit/scripts/"
    fi

    # Install templates
    if [[ -d "$source_dir/.mykit/templates" ]]; then
        mkdir -p "$install_dir/.mykit/templates"
        cp -r "$source_dir/.mykit/templates"/* "$install_dir/.mykit/templates/"
    fi
